...
export class UserController {
  ...
  public async createUser(
    @Body({ validate: true }) userRequest: any,
    @Res() response: any
  ): Promise<any> {
    ...
    if (ifEmail) {
      return response
        ...
        .send({
          status: 0,
          message:
            'The given email already exists. Please try another email :)',
        });
    }

    const ifFindMobile = await adminUserModels.findOne({
    ...
    if (ifFindMobile) {
      return response
        ...
        .send({
          status: 0,
          message:
            'The given mobile number already exists. Please try another mobile number :)',
        });
    }
    ...
    if (ifEmail && ifFindMobile) {
      return response
        ...
        .send({
          status: 0,
          message:
            'The given email and mobile number already exists. Please try another email and mobile number :)',
        });
    }
    ...
    newUser.isActie = 0;
    (newUser.isDelete = 0), (newUser.firstName = userRequest.firstName);
  ...
  public async login(
    ...
    if (!findUser) {
      return response
        .status(400)
        .send({ status: 0, message: 'Invalid username !!' });
    }
    ...
    const tokens = await jsonwebtoken.sign(
      { userId: findUser._id, role: 'admin-user' },
      'fsha%@%xcb754wejh'
    );
    ...
    return response
      .status(400)
      .send({ status: 0, message: 'Invalid login !!' });
  }
  ...
  public async getUser(
    @QueryParam('universityName') universityNames: string,
    @QueryParam('collegeName') collegeNames: string,
    ...
    const userData = await adminUserModels
      .find(findOperation)
      .limit(limit)
      .skip(offset);
    const successResponse = {
  ...
  public async updateUser(
    ...
    if (!ifUser) {
      return response
        .status(400)
        .send({ status: 0, message: 'Invalid user Id !!' });
    }
    ...
    return response
      .status(400)
      .send({ status: 0, message: 'Unable to update the User !!' });
  }
  ...
  public async userDetail(
    ...
    if (!ifUser) {
      return response
        .status(400)
        .send({ status: 0, message: 'Unable to get the user detail !!' });
    }
  ...
  public async deleteUser(
    ...
    if (!ifUser) {
      return response
        .status(400)
        .send({ status: 0, message: 'Inlid user is !!' });
    }
    ...
    return response
      .status(400)
      .send({ status: 0, message: 'Unable to deleted a User !!' });
  }
  @Post('/send')
  public async sendMail(@Res() response: any): Promise<any> {
    ...
    return response
      .status(200)
      .send({ status: 1, message: 'Successfully send the mail !!' });
  ...
  public async uploadImage(
    ...
    const uploadImage = await this.imageSerice.uploadImage(
      'uploads',
      bufferData,
      type
    );
    if (uploadImage) {
      return response
        .status(200)
        .send({ status: 1, message: 'Successfully upload the image !!' });
    }
    return response
      .status(400)
      .send({ status: 0, message: 'Invalid directory name !!' });
  }
  ...
  public async rabbitMqExch(@Res() res: any): Promise<any> {
    ...
    return res
      .status(200)
      .send({ status: 1, message: 'Successfully sent to the queue' });
  }
  // convert html to pdf
  @Get('/html-pdsf')
  ...
  @Get('/create-razorpay-account')
  public async createRazorPayAccount(@Res() respose: any): Promise<any> {
    try {
      //     const Razorpay = require("razorpay");
      //     const razorpay = new Razorpay({
      //         key_id: "rzp_test_3NReBoXTAPenIP",
      //         key_secret: "yrEWRypjXxYHDWLFcldZV1Bc",
      //    });
      ...
      //     }
      //   );
      const response = await axios.get('https://api.razorpay.com/v1/accounts', {
        auth: {
          username: 'rzp_test_3NReBoXTAPenIP',
          password: 'yrEWRypjXxYHDWLFcldZV1Bc',
        },
      });
      ...
      return respose
        ...
        .send({
          status: 1,
          message: 'Successfully created a Seller Account',
          data: response,
        });
    } catch (error) {
      ...
      return respose
        .status(500)
        .send({ status: 0, message: 'Error Registering Seller', error });
    }
  }

  @Post('/razorpay-order')
  public async razorpayOrder(
    @Body({ validate: true }) payload: any,
    @Res() response: any
  ): Promise<any> {
    try {
      const Razorpay = require('razorpay');
