...
import { chat } from '../../Models/MasterModel';
...
export class ChatController {
  ...
  // Create chat data
  @Post()
  public async createChatData(
    @BodyParam('senderData') senderData: any,
    @BodyParam('receiverData') recevier: string,
    @Res() response: any
  ): Promise<any> {
  ...
  public async getChatData(
    ...
    const getData = await chat.find({
      sender: { $regex: keyword, $options: 'i' },
    });
    if (getData) {
    ...
    if (orginalFile === 'zip') {
      ...
      zip.on('ready', () => {
        for (const entries of Object.values(zip.entries())) {
          ...
          if (
            fileNames.includes(Object.values(entries)[16].split('.')[1]) ===
            false
          ) {
        ...
        .catch((err) => {
          if (err) {
            return response.status(400).send({
              status: 400,
              message: 'Can not able to extract the zip file !!',
              data: err,
            });
          }
        });
      ...
      if (errorData.length > 0) {
        return response
          .status(400)
          .send({ status: 0, message: 'Inavlid files', data: errorData });
      }
      ...
      const xlToJson = await this.bulkImportService.xlsxToJson(
        inputFile,
        'Sheet1'
      );
      ...
      return response
        .status(200)
        .send({ status: 1, message: 'Success...!!!', data: xlToJson });
    }
    return response
      .status(400)
      .send({ status: 0, message: 'Just give zip file extension !!' });
  }
}
